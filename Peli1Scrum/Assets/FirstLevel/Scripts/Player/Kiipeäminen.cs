...
public class Kiipeäminen : MonoBehaviour
{
    ...
    //Tän voi vaihtaa ainakin ylös kiipeemiseen ja kiipeilystä tiputtautumiseen, mut eka ottaa tulevapaikka2 pois käytöstä
    public void Nollaa()
    {
        roiku = false;
        liiku = false;
    ...
    //Tulee käyttöön myöhemmin
    public static  bool Castaa(Ray ray, out RaycastHit raycastHit)
    {
    ...
    //Vois muuttaa näitä metodeiks
    void Update()
    {
        //Tulee käyttöön myöhemmin
        //RaycastHit hittiii;
        //Ray rayTesti = new Ray(Ray0Origin.position, Ray0Origin.forward);
        //if (Castaa( rayTesti, out hittiii))
        //{
        //    UnityEngine.Debug.Log("Osui"+ hittiii.point);
        //}
        //else
        //{
        ...
        else if (Input.GetKeyDown("e") && liiku == false && roiku == false && eka == false && cc.isGrounded == true)
        {
            ...
            if (Physics.Raycast(ray0, out kiipeemisenAlotusHit, maxDistance: 0.5f))
            {
                        ...
                        kiipeemisenAlotusHit.collider.tag != "Lava" && kiipeemisenAlotusHit.collider.tag != "Moukari")
                {
                    ...
                    var tulevaSuuntaToisinpäinNormaaliString = tulevaSuuntaToisinpäin.normalized.ToString();

                    var hitinsuuntaString = kiipeemisenAlotusHit.normal.normalized.ToString();
        ...
        if (roiku && transform.rotation.normalized == oikeaSuunta.normalized)
        {
            ...
            //Tällä pidetään pelaaja oikeassa kohdassa roikkumassa kun kiipeillään. Esim. jos tasanne on kalteva ja liikutaan sitä pitkin sivuttain, niin korjataan korkeutta
            ...
            //Nämä tapahtuu eka, kun aloitetaan roikkuminen
            else
            {
                liiku = true;
    ...
    private void kiipeilyVasemmalle()
    {
        ...
        if (transform.position == korjattuRoikkumisPaikka && transform.rotation == oikeaSuunta)
        {
            ...
            //Tsekataan onko vasemmalla seinä, jos on niin käännytään sitä kohti ja jos osuu niin että tulevalla suunnalla on tasanne josta roikkua
            ...
            else if (!Physics.CapsuleCast(point1, point2, radius, -transform.right, castDistance))
            {
                ...
                //Tsekataan onko seinää vasemmalle+0.1 yksikköä ja onko siinä tasannetta ja sen tasanteen kulma, että ei ole liian jyrkkä
    ...
    private void kiipeilyOikealle()
    {
        ...
        if (transform.position == korjattuRoikkumisPaikka && transform.rotation == oikeaSuunta)
        {
            ...
            //Tsekataan onko oikealla seinä, jos on niin käännytään sitä kohti ja jos osuu niin että tulevalla suunnalla on tasanne josta roikkua
            ...
            {
                //Tsekataan onko seinää oikealle +0.1 yksikköä ja onko siinä tasannetta ja sen tasanteen kulma, että ei ole liian jyrkkä
    ...
    private void kiipeysRoikkumisesta()
    {
        ...
        //Jos pisteestä raynormal origin +1 ylöspäin ray +0.5 eteenpäin ei osu, eli ei ole seinää tasanteen päällä 
        if (Physics.Raycast(rayNormalTulevaPositio, maxDistance: 0.5f, hitInfo: out rr) == false)
        {
            ...
            {
                Vector3 capsulenLähtöPaikka = (rayNormalTulevaPositio1Hit.point + (Vector3.up * 0.05f) - (transform.forward.normalized * 0.7f));
                float raycapsulenMatkaKeskeltäPointteihin = col.height / 2 - col.radius;
    ...
    private void tipuRoikkumisesta()
    {
        Nollaa();
    }
    ...
    //Kiipeily tapahtuu periaatteella: jos eka ray joka lähtee melkein maan tasosta y-suunnassa ylöspäin osuu ja aiempi ray osuu ja nykyinen ray ei osu
